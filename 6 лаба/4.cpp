...
public:
    ...
    {
        queue.push((uint16_t)(val % max));
    }
};

void reading(ifstream & input, vector<string> & program)
{
    string line;
...
void searching(vector<string> & source, unordered_map<string, int> & labels)
{
...
void go(vector<string> & source, ofstream & output_stream)
{
    Queue queue;
    unordered_map<string, int> labels;
    vector<int64_t> registers(26);
    searching(source, labels);
